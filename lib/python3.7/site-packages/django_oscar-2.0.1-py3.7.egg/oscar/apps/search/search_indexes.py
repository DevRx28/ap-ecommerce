...
class ProductIndex(indexes.SearchIndex, indexes.Indexable):
    ...
    brand = indexes.MultiValueField(null=True, faceted=True)
    ...
    attribute1 = indexes.MultiValueField(null=True, faceted=True)
    attribute2 = indexes.MultiValueField(null=True, faceted=True)
    attribute3 = indexes.MultiValueField(null=True, faceted=True)
    ...
    def prepare_category(self, obj):
        ...
        if len(categories) > 0:
            return [category.full_name for category in categories if 'Brand' not in category.full_name]
    
    def prepare_brand(self, obj):
        categories = obj.categories.all()
        if len(categories) > 0:
            categ = [category.full_name for category in categories if 'Brand' in category.full_name]
            if 'Acer' in categ[0]:
                brand_nam = categ[0][-5:]
            elif 'Dell' in categ[0]:
                brand_nam = categ[0][-5:]
            elif 'Samsung' in categ[0]:
                brand_nam = categ[0][-8:]
            elif 'LG' in categ[0]:
                brand_nam = categ[0][-3:]
            return brand_nam
    ...
    def prepare_attribute1(self, obj):
        attribute_values = obj.attribute_values.all()
        if len(attribute_values) > 0:
            return attribute_values[0].value
    
    def prepare_attribute2(self, obj):
        attribute_values = obj.attribute_values.all()
        if len(attribute_values) > 0:
            return attribute_values[1].value

    def prepare_attribute3(self, obj):
        attribute_values = obj.attribute_values.all()
        if len(attribute_values) > 0:
            return attribute_values[2].value    
